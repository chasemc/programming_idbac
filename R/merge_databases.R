...
merge_idbac_experiments <- function(existing_db_paths,
  ...
  for (i in seq_along(existing_db_paths_names)) {
    ...
    new_pool <- IDBacApp::createPool(new_db_name,
                                     new_db_path)[[1]]

    existing_pool <- IDBacApp::createPool(existing_db_paths_names[[i]],
                                          dirname(existing_db_paths[[i]]))[[1]]

    con <- pool::poolCheckout(existing_pool)
    samp_names <- IDBacApp::availableSampleNames(con,allSamples = T)
    pool::poolReturn(con)
...
merge_idbac_experiments(existing_db_paths = list.files("C:/Users/CMC/Downloads/ro1_rmd",
                                                       pattern = "sqlite",
                                                       full.names = TRUE),
                        new_db_name = "bigGuy",
                        new_db_path = "C:/Users/CMC/Documents/big_r01")
